...
J = - sum(sum(recodedY .* log(hypothesis) + (1 - recodedY) .* log(1 - hypothesis))) / m;
regularizationTerm = (lambda / (2*m)) * (sum(sum(Theta1(:,2:end).^2)) + sum(sum(Theta2(:,2:end).^2)));
J = J + regularizationTerm;

% Part 2: Backprop
error_3 = hypothesis - recodedY;
error_2 = (error_3 * Theta2(:,2:end)) .* sigmoidGradient(z2);
Theta1_grad = (Theta1_grad + error_2'*a1) / m;
Theta2_grad = (Theta2_grad + error_3'*a2) / m;
